CREATE OR REPLACE FUNCTION set_task_as_completed
(
...
BEGIN
  IF (SELECT t."status"
       ...
       WHERE t."owner_uuid" = p_owner_id
         ...
         AND t."task_id" = p_task_id) = 'finished'::task_status_t
  THEN
    RETURN FALSE;
  END IF;
  UPDATE "task"
...
ALTER FUNCTION set_task_as_completed ("task"."owner_uuid"%TYPE,
                                      "task"."task_id"%TYPE,
                                      "task"."task_id"%TYPE)
      OWNER TO "noda";
